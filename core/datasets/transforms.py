    ...
    def __call__(self, results):
        for key in results.get('img_fields', ['img']):
            ...
            if img_mask is not None:
                img_mask = imcrop(img_mask, bboxes=np.array([xmin, ymin, xmax, ymax]))
                results['img_mask'] = imresize(
                    img_mask,
                    tuple(self.size[::-1]),
                    interpolation=self.interpolation,
                    backend=self.backend)
            ...
            for key in results.get('img_fields', ['img']):
                ...
                if img_mask is not None:
                    results['img_mask'] = imflip(
                        img_mask, direction=results['flip_direction'])
        return results
...
class CenterCrop(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            if img_mask is not None:
                img_mask = imcrop(img_mask, bboxes=np.array([x1, y1, x2, y2]))
...
class Normalize(object):
    ...
    def __call__(self, results):
        for key in results.get('img_fields', ['img']):
            ...
            if 'img_mask' in results:
                img_mask = results['img_mask']
                # results['img_mask'] = imnormalize(results['img_mask'], self.mean, self.std,
                #                            self.to_rgb)
                results['img_mask'] = img_mask
