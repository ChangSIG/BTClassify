...
def train(model, runner, lr_update_func, device, epoch, epoches, meta):
    ...
    with tqdm(total=len(runner.get('train_loader')), desc=f'Train: Epoch {epoch + 1}/{epoches}', postfix=dict,
        ...
        for iter, batch in enumerate(runner.get('train_loader')):
            images_mask = None
            if len(batch) == 4:
                images, images_mask, targets, _ = batch
                with torch.no_grad():
                    images = images.to(device)
                    images_mask = torch.div(images_mask, 255.0).to(torch.float32).to(device)
            ...
            if images_mask is not None:
                losses = model(images, images_mask=images_mask, targets=targets, return_loss=True)
            else:
    ...
    with torch.no_grad():
        with tqdm(total=len(runner.get('val_loader')), desc=f'Test : Epoch {epoch + 1}/{epoches}',
            ...
            for iter, batch in enumerate(runner.get('val_loader')):
                if len(batch) == 4:
                    images, images_mask, target, _ = batch
                    with torch.no_grad():
                        images = images.to(device)
                        images_mask = torch.div(images_mask, 255.0).to(torch.float32).to(device)
                        target = target.to(device)
                    outputs = model(images.to(device), torch.div(images_mask, 255.0).to(torch.float32).to(device),
                                    return_loss=False)
                else:
